...
class TestMailhogEmailSender:
    ...
    @pytest.fixture
    def email_sender(self, smtp_config):
        return MailhogEmailSender(smtp_config)

    def test_send_activation_email_success(self, email_sender):
        mock_smtp = MagicMock()
    ...
    def test_send_activation_email_failure(self, email_sender):
        mock_smtp = MagicMock()
        mock_smtp.__enter__.return_value = mock_smtp
        ...
        with patch("smtplib.SMTP", return_value=mock_smtp):
            with pytest.raises(EmailDeliveryException) as exception:
                email_sender.send_activation_email(
                    Email("test@spookymotion.com"), "1234"
                )
